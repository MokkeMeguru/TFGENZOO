...
def bits_x(
    ...
    formula:
    - (log_likelihood + log_det_jacobian) / (log 2 * h * w * c) + log(2^n_bits) / log(2.)
    """
...
class Glow_trainer:
    def __init__(self, args=args.args, training=True):
        ...
        self.train_log_det_jacobian = Mean(name="log_det_jacobian", dtype=tf.float32)
        self.valid_log_det_jacobian = Mean(name="log_det_jacobian", dtype=tf.float32)
        ...
        # setup checkpoint
        checkpoint_path = args.get("checkpoint_path", "checkpoints/glow")
        self.setup_checkpoint(checkpoint_path)

        if training:
            for sample in self.train_dataset.take(1):
                print(
                    "init loss (log_prob bits/dims) {}".format(
    ...
    def setup_checkpoint(self, checkpoint_path: Path):
        ...
        ckpt = tf.train.Checkpoint(
            step=tf.Variable(1), model=self.glow, optimizer=self.optimizer
        )
        ...
        if ckpt_manager.latest_checkpoint:
            ...
            for sample in self.train_dataset.take(1):
                print(
                    "restored loss (log_prob bits/dims) {}".format(
                        tf.reduce_mean(self.val_step(sample["img"]))
                    )
                )
        self.ckpt = ckpt
    ...
    def generate_image(self, beta_z: float = 0.2, beta_zaux: float = 0.2):
        ...
        with self.writer.as_default():
            ...
            tf.summary.image(
                "generated image", x, max_outputs=4, step=self.optimizer.iterations
            )
            for x in self.test_dataset.take(1):
                ...
                z, log_det_jacobian, zaux, ll = self.glow(x["img"][:4], training=False)
    ...
    def train(self, beta_z=0.2, beta_zaux=0.2):
        for epoch in range(self.args["epochs"]):
            ...
            tf.summary.scalar(
                "train/nll", self.train_loss.result(), step=self.optimizer.iterations
            )
            tf.summary.scalar(
                "valid/nll", self.valid_loss.result(), step=self.optimizer.iterations
            )
            tf.summary.scalar(
                "train/ldj",
            ...
            print(
                "epoch {}: train_loss={}, val_loss={}, saved_at={}".format(
                    epoch,
